...
const hideAllLocationIcons = (varsState) => {
  for (const locId in varsState.locations) {
    varsState.locations[locId].display = false;
  }
};
...
const setMapLegendIcons = (icons, iconsOrder, varsState) => {
  ...
  // fill lists
  if (!iconsOrder) {
    for (const [curModuleInstanceId, curModuleInstanceValue] of Object.entries(
      icons
    )) {
...
const setUniformIcon = (iconUrl, varsState) => {
  for (const locId in varsState.locations) {
    varsState.locations[locId].icon = iconUrl;
  }
};
...
const setTimeSeriesPlotAvailableVariables = (availableVariables, varsState) => {
  varsState.domObjects.timeSeriesData.availableVariables = availableVariables;
};
...
const setTimeSeriesPlotModelEvaluationMetricsUrls = (
  evaluationMetricsUrls,
  varsState
) => {
...
const setTimeSeriesPlotModelEvaluationMetrics = (
  evaluationMetrics,
  varsState
) => {
...
const updateLocationIcons = (varsState, consCache, consFixed, settings) => {
  ...
  if (inMainMenuControlActiveTabOverview(varsState)) {
    ...
    if (getContextIconsType(varsState) === "uniform") {
      ...
      _updateLocationIconsCompetition(
        varsState,
        consCache,
        consFixed,
        settings
...
const _replaceUrlParam = (url, paramName, paramValue) => {
  ...
  url = url.replace(/[?#]$/, "");
  return (
    url + (url.indexOf("?") > 0 ? "&" : "?") + paramName + "=" + newParamValue
  );
};

const _showAllLocationIcons = (varsState) => {
  for (const lcId in varsState.locations) {
    varsState.locations[lcId].display = true;
  }
};

const _setAllLocationsUniformIcons = (iconUrl, varsState) => {
  for (const lcId in varsState.locations) {
    varsState.locations[lcId].icon = iconUrl;
  }
};

//
const _updateLocationIconsAlerts = (
  varsState,
  consCache,
  consFixed,
  settings
  ...
  const moduleInstanceId = getContextIconsArgs(
    "alerts",
    varsState
  ).moduleInstanceId;
  const thresholdGroupId = getContextIconsArgs(
    "alerts",
    varsState
  ).thresholdGroupId;

  // get timeseries
  const consideredTimeseries = consCacheLib.getTimeseriesIdsInFilterId(
    filterId,
    consCache
  );
  ...
  const allParameterIds = consCacheLib.getParameterIdsByThresholdGroupId(
    thresholdGroupId,
    consCache
  );

  // get all info about thresh group
  const thresholdGroupData = consFixedLib.getThresholdGroupData(
    thresholdGroupId,
    consFixed
  );
  const thresholdGroupBase = consFixedLib.getThresholdGroupBaseIcons(
    thresholdGroupId,
    settings
  );
  ...
  for (const curThreshLevelObj of thresholdGroupData.threshold_levels) {
    ...
    thresholdLevelFunction[curThreshLevelId] =
      consFixedLib.getThresholdLevelData(
        curThreshLevelId,
        consFixed
      ).valueFunction;
  }
  ...
  const getThresholdLevel = (
    ...
    for (const curThreshLevelObj of thresholdGroupData.threshold_levels) {
      ...
      if (
        curThreshLevelFunction.charAt(0) === "@" &&
        curThreshLevelFunction.charAt(curThreshLevelFunction.length - 1) === "@"
      ) {
        // get the value of the attribute in the location info
        const curAttrId = curThreshLevelFunction.substring(
          1,
          curThreshLevelFunction.length - 1
        );
        const curAttrObj = _getObjectFromArrayById(
          curAttrId,
        ...
        if (timeseriesData.maxValue < curAttrValue) {
          break;
        }
        lastThreshLevelObj = curThreshLevelObj;
      } else {
        console.warn(
          "Unexpected threshold value function:",
          curThreshLevelFunction
        );
      }
    }
  ...
  for (const curTimeseriesId of consideredTimeseries) {
    const curTimeseriesData = consCacheLib.getTimeseriesData(
      curTimeseriesId,
      consCache
    );
    if (
      allParameterIds.has(curTimeseriesData.header.parameterId) &&
      ...
      const curLocationData = consFixedLib.getLocationData(
        curLocationId,
        consFixed
      );
      ...
      varsState.locations[curTimeseriesData.header.location_id].icon =
        selectedIcon;
    }
  }
...
//
const _updateLocationIconsComparison = (
  varsState,
  consCache,
  consFixed,
  settings
) => {
  const comparisonsArgs = varsStateLib.getContextIconsArgs(
    "comparison",
    varsState
  );
  ...
  for (const curTimeseriesId of consideredTimeseries) {
    ...
    const curParameterGroupId = consFixedLib.getParameterGroupOfParameterId(
      curTimeseriesData.header.parameterId,
      consFixed
    );
    const curLocationId = curTimeseriesData.header.location_id;
    ...
    if (curParameterGroupId != selectedParameterGroupId) {
      continue;
    }
    if (!comparisonsArgs.moduleInstanceIds.has(curModuleInstanceId)) {
      continue;
    }
  ...
  for (const [curLocationId, curLocationDict] of Object.entries(
    locationIdsIcons
  )) {
    ...
    for (const [curModuleInstanceId, curModuleInstanceValue] of Object.entries(
      ...
      if (!curWinModuleInstanceId) {
        ...
        _isComparisonWinner(selectedMetric, curWinValue, curModuleInstanceValue)
      ) {
        curWinModuleInstanceId = curModuleInstanceId;
        curWinValue = curModuleInstanceValue;
      }
    ...
    const winIconUrl =
      useIcons[selModInstIdsArray.indexOf(curWinModuleInstanceId)];
  ...
  for (const curModuleInstanceId of comparisonsArgs.moduleInstanceIds) {
    const curIconUrl =
      useIcons[selModInstIdsArray.indexOf(curModuleInstanceId)];
...
//
const _updateLocationIconsCompetition = (
  varsState,
  consCache,
  consFixed,
  settings
  ...
  // get last response
  const lastUrlResponseData =
    consCacheLib.getCompetitionLastResponseData(consCache);
  if (!lastUrlResponseData) {
    console.log("Did not stored urlResponseData from", consCache);
    return;
  }

  const competitionArgs = varsStateLib.getContextIconsArgs(
    "competition",
    varsState
  );
  const allIcons = settings.locationIconsOptions.comparison.icons; // TODO - change to competition
  const selModInstIdsArray = Array.from(
    competitionArgs.simulationModuleInstanceIds
  );
  const useIcons = allIcons.slice(0, selModInstIdsArray.length);
  ...
  for (const curLocationId in varsState.locations) {
    ...
    for (const [curModuleInstanceId, curModuleInstanceDict] of Object.entries(
      lastUrlResponseData.locations[curLocationId].simulations
    )) {
      const curModuleInstanceValue = curModuleInstanceDict.value;
      if (curWinValue && curModuleInstanceValue < curWinValue) {
        continue;
      }
      [curWinSimulation, curWinValue] = [
        curModuleInstanceId,
        curModuleInstanceValue,
      ];
    }
...
const _updateLocationIconsUniform = (varsState, consCache, settings) => {
  ...
  const filterId = getContextFilterId(varsState);
  const timeseriesIdsByFilter = consCacheLib.getTimeseriesIdsInFilterId(
    filterId,
    consCache
  );
  ...
  const locationIdsByFilter = new Set(
    Array.from(timeseriesIdsByFilter).map((timeseriesId) => {
  ...
  setUniformIcon(settings.generalLocationIcon, varsState);
  for (const locationId in varsState.locations) {
    varsState.locations[locationId].display =
      locationIdsByFilter.has(locationId);
  }
  varsStateLib.setMapLegendSubtitle("Uniform:", varsState);
  varsStateLib.setMapLegendIcons(
    { Location: settings.generalLocationIcon },
    null,
    varsState
  );
  varsStateLib.setMapLegendVisibility(true, varsState);
...
const _updateLocationIconsEvaluation = (varsState, consCache, settings) => {
  // get last response
  const lastUrlResponseData =
    consCacheLib.getEvaluationLastResponseData(consCache);
  if (!lastUrlResponseData) {
    return;
  }
  ...
  const iconOptions =
    settings.locationIconsOptions.evaluation.options[lastUrlResponseData.metric]
      .parameterGroups["Discharge"]; // TODO: make it flexible
  ...
  for (const locationId in varsState.locations) {
    ...
    if (!lastUrlResponseData.locations[locationId]) {
      ...
      while (rangeTopIdx < iconOptions.ranges.length) {
        ...
        if (
          !curTopVal ||
          lastUrlResponseData.locations[locationId].value <= curTopVal
        ) {
...
const varsStateLib = {
  ...
  inMainMenuControlActiveTabActiveFeatureInfo:
    inMainMenuControlActiveTabActiveFeatureInfo,
  ...
  setMainMenuControlActiveTabAsActiveFeatureInfo:
    setMainMenuControlActiveTabAsActiveFeatureInfo,
  ...
  setTimeSeriesPlotModelEvaluationMetrics:
    setTimeSeriesPlotModelEvaluationMetrics,
  setTimeSeriesPlotModelEvaluationMetricsUrls:
    setTimeSeriesPlotModelEvaluationMetricsUrls,
