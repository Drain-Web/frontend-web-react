...
const displayPolygon = (polygonId, varsState) => {
  ...
  if (
    varsStateLib.inMainMenuControlActiveTabActiveFeatureInfo(varsState) &&
    varsStateLib.getLastActiveTab(varsState) === "tabOverview"
  ) {
    return true;
  } else {
    return (
      varsStateLib.getContextFilterGeoId(varsState) === polygonId ||
    ...
    <LayersControl.Overlay checked name={layerName}>
      <LayerGroup name={layerName}>
        {layerData.map((poly) => {
          ...
          /* build polygons */
          return displayPolygon(poly.id, varsState) && polygon ? (
            <Polygon
              pathOptions={{
